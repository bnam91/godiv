...
### CDP 모드 (봇차단이 심할 때 — 권장 우회)

생 프로필로 막히면, **이미 로그인된 실제 크롬에 붙어서** 다운로드한다. 로그인 세션 + 실제 브라우저 지문이라 안티봇 통과율이 크게 오른다(`div_download` 스킬과 같은 방식). 검증됨: 추출→다운로드→저장 전 구간 동작.

1. 로그인된 크롬을 원격 디버깅 포트로 실행 (예):
   ```bash
   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
     --remote-debugging-port=9222 --user-data-dir="$HOME/godiv-chrome"
   ```
   (그 창에서 네이버/쿠팡에 로그인. 다음부턴 유지됨.)
2. godiv 좌측 패널 → **"고급 · 봇차단이 심할 때 (CDP 모드)"** 펼치기 → 체크 + 포트(9222) 입력.
3. 다운로드하면 그 크롬 세션으로 진행. (godiv는 그 크롬을 닫지 않고 attach만 함)

> ⚠️ 다른 세션이 쓰는 CDP 포트(예: div_download의 9225/9333)와 충돌하지 않게 별도 포트를 쓸 것.
